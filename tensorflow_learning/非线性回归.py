...
with tf.Session() as sess:
    ...
    for epoch in range(10):
        x_in = sess.run(tf.convert_to_tensor(c[i]))
        y_in = sess.run(tf.convert_to_tensor(e[i]))
        z_in = sess.run(tf.convert_to_tensor(F))
        for i in range(850):
            sess.run(opti,feed_dict={x:[x_in],y:[y_in]})
        for i in range(850,900):
            data=sess.run(result,feed_dict={x:[x_in],z:[z_in]})
